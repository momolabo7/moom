...
#define GBG_TILE_SIZE (30)
#define GBG_TILE_SIZE_HALF (GBG_TILE_SIZE/2)
#define GBG_GRID_WIDTH 128
#define GBG_GRID_HEIGHT 128
...
tbp_update_and_render_grid_stuff(eden_t* eden, tbp_t* tbp)
{
  ...
  const f32_t start_x = -GBG_DESIGN_WIDTH/2 + GBG_TILE_SIZE_HALF;
  const f32_t start_y = -GBG_DESIGN_HEIGHT/2 + GBG_TILE_SIZE_HALF;
  f32_t current_x = start_x;
  f32_t current_y = start_y;


  for_cnt(row, GBG_GRID_HEIGHT) // row
  {
    for_cnt (col, GBG_GRID_WIDTH) // col
    {
      if (tbp->cells[row][col] == 1)
      {
        rgba_t color = RGBA_WHITE;
        f32_t widths = GBG_TILE_SIZE_HALF + GBG_TILE_SIZE_HALF;
        ...
        {
          f32_t heights = GBG_TILE_SIZE_HALF + GBG_TILE_SIZE_HALF;
...
eden_update_and_render_sig(eden_update_and_render) 
{
  ...
  eden_set_view(eden, -GBG_DESIGN_WIDTH/2, GBG_DESIGN_WIDTH/2, -GBG_DESIGN_HEIGHT/2, GBG_DESIGN_HEIGHT/2, 0.f, 0.f);
  ...
  // update player
  {
    ...
    // lupsup collision
    {
      ...
      f32_t feet = tbp->player.pos.y - GBG_TILE_SIZE_HALF;
