                   ...
                   Gfx* gfx) 
{
      ...
      ++bitmap_index) 
  {
    ...
    gfx_push_delete_texture(gfx, bmp->renderer_texture_handle);
  }
}
...
{ 
  ...
  {
    ga->bitmaps = ba_push_arr(Bitmap_Asset, allocator, ga->bitmap_count);
    for(U32 bitmap_index = 0; 
        bitmap_index < ga->bitmap_count; 
        ++bitmap_index) 
    {
      ...
      pf->read_file(file, sizeof(Karu_Bitmap), offset, &kb);
      
      U32 bitmap_size = kb.width*kb.height*4;
      U32 texture_handle = get_next_texture_handle();  
      
      auto* payload = gfx_begin_texture_transfer(gfx, bitmap_size);
  ...
  {
    ga->sprites = ba_push_arr(Sprite_Asset, allocator, ga->sprite_count);
        ...
        ++sprite_index) 
    {
      ...
      pf->read_file(file, sizeof(Karu_Sprite), offset, &ks);
  ...
  if (ga->font_count > 0) 
  {
    ga->fonts = ba_push_arr(Font_Asset, allocator, ga->font_count);
        ...
        ++font_index) 
    {
      ...
      pf->read_file(file, sizeof(Karu_Font), kf_offset, &kf);
      ...
      void* font_memory = ba_push_size(allocator, memory_required, 4);
          ...
          ++glyph_index) 
      {
        ...
        pf->read_file(file, sizeof(Karu_Font_Glyph), kfg_offset, &kfg);
      ...
      // horizontal advances
      {
        ...
        pf->read_file(file, block_size, advance_offset, font->horizontal_advances);
