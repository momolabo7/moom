...
begin_packer(const char* pack_id_filename,
  ...
  assert(ret.pack_id_file);
  assert(ret.bitmap_id_file);
  assert(ret.font_id_file);
  assert(ret.sprite_id_file);
  
  // boiler plate:
  {
    String boiler = create_string_from_lit("// Generated by sui\n");
    fwrite(boiler.e, boiler.count, 1, ret.pack_id_file);
    fwrite(boiler.e, boiler.count, 1, ret.bitmap_id_file);
    fwrite(boiler.e, boiler.count, 1, ret.font_id_file);
    fwrite(boiler.e, boiler.count, 1, ret.sprite_id_file);
  }
  
  
  String str = {};
  str = create_string_from_lit("enum Pack_ID {\n");
  fwrite(str.e, str.count, 1, ret.pack_id_file);
  str = create_string_from_lit("enum Bitmap_ID {\n");
  fwrite(str.e, str.count, 1, ret.bitmap_id_file);
  str = create_string_from_lit("enum Font_ID {\n");
  fwrite(str.e, str.count, 1, ret.font_id_file);
  str = create_string_from_lit("enum Sprite_ID {\n");
  fwrite(str.e, str.count, 1, ret.sprite_id_file);
...

{
       ...
       ++bitmap_index) 
  {
    ...
    String format = create_string_from_lit("%s = %s << 16 | %u,\n");
  ...
  // Write the pack id
  {
    String format = create_string_from_lit("%s = %u,\n");
