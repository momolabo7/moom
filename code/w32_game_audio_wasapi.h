...
struct w32_wasapi_t {
  game_audio_buffer_t pf_audio; // Must be first member
...
w32_wasapi_end_frame(w32_wasapi_t* wasapi) 
{
  ...
  game_audio_buffer_t* output = &wasapi->pf_audio;
...
static void
w32_audio_begin_frame(game_audio_buffer_t* audio) {
  w32_wasapi_begin_frame((w32_wasapi_t*)audio);
}

static void 
w32_audio_end_frame(game_audio_buffer_t* audio) {
  w32_wasapi_end_frame((w32_wasapi_t*)audio);
}

static void
w32_audio_unload(game_audio_buffer_t* audio) {
  // Unused
}
